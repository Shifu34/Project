...
export const registerPatient = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const dup = await query(
      `SELECT id FROM users WHERE email = $1 OR (cnic = $2 AND cnic IS NOT NULL)`,
      [email, cnic ?? null],
    );
    ...
    try {
      ...
      const userRes = await client.query(
        `INSERT INTO users (role_id, first_name, last_name, email, password_hash, phone, cnic, is_active)
         ...
         RETURNING id, email, first_name, last_name, phone, cnic`,
        [patientRole.id, first_name, last_name, email, hash, phone, cnic ?? null],
      );
      ...
      if (cnic) {
        ...
        if (existing.rows.length > 0) {
          ...
          const updated = await client.query(
            `UPDATE patients SET user_id = $1, email = $2, phone = $3, updated_at = CURRENT_TIMESTAMP
             WHERE cnic = $4 RETURNING patient_code`,
            [newUser.id, email, phone, cnic],
          );
      ...
      if (!patientCode) {
        ...
        const ins = await client.query(
          `INSERT INTO patients (user_id, first_name, last_name, email, phone, gender, date_of_birth, cnic)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING patient_code`,
          [newUser.id, first_name, last_name, email, phone, gender ?? null, date_of_birth, cnic ?? null],
        );
    ...
    if (user.role_name === 'patient') {
      if (user.cnic) {
        const patientRes = await query(
          `SELECT id, gender, date_of_birth FROM patients WHERE cnic = $1 LIMIT 1`,
          [user.cnic],
        );
        if (patientRes.rows.length > 0) {
          ...
          await query(
            `UPDATE patients SET user_id = $1, email = $2, phone = $3, updated_at = CURRENT_TIMESTAMP
             WHERE cnic = $4 AND (user_id IS NULL OR user_id = $1)`,
            [user.id, user.email, user.phone, user.cnic],
          );
        }
      } else {
        const patientRes = await query(
...
export const getMe = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result = await query(
      `SELECT u.id, u.first_name, u.last_name, u.email, u.phone, u.cnic,
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result = await query(
      `SELECT id, first_name FROM users WHERE email = $1 AND is_active = true LIMIT 1`,
      [email],
    );
    ...
    await sendPasswordResetCode(email, code, user.first_name);
...
export const resendResetCode = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { email } = req.body;
...
export const resetPassword = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    await query(`UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`, [hash, user_id]);
...
export const sendRegistrationOtp = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const recent = await query(
      `SELECT created_at FROM email_verification_tokens WHERE email = $1 ORDER BY created_at DESC LIMIT 1`,
      [email],
    );
    if (recent.rows.length > 0) {
      const secondsSinceLast = (Date.now() - new Date(recent.rows[0].created_at).getTime()) / 1000;
    ...
    await query(`DELETE FROM email_verification_tokens WHERE email = $1`, [email]);
    await query(
      `INSERT INTO email_verification_tokens (email, code, expires_at) VALUES ($1, $2, $3)`,
