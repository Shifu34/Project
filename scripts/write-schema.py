#!/usr/bin/env python3
"""One-time helper: writes the canonical schema.sql for Murshid Hospital."""

SCHEMA = r"""-- =============================================================
-- Murshid Hospital System - PostgreSQL Database Schema
-- 34 Tables — matches ERD spec exactly
-- =============================================================
...
import os
dest = os.path.join(os.path.dirname(__file__), '..', 'database', 'schema.sql')
dest = os.path.normpath(dest)
with open(dest, 'w') as f:
    f.write(SCHEMA)

tables = [l for l in SCHEMA.splitlines() if l.startswith('CREATE TABLE')]
print(f"Written {len(tables)} tables to {dest}")
for t in tables:
    print(' ', t)
