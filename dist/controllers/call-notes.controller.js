...
exports.getCallNotes = exports.getCallNoteById = exports.createCallNote = void 0;
...
exports.createCallNote = createCallNote;
// ─────────────────────────────────────────────────────────────────────────────
// GET /calls/notes/:id
// ─────────────────────────────────────────────────────────────────────────────
const getCallNoteById = async (req, res, next) => {
    try {
...
exports.getCallNoteById = getCallNoteById;
// ─────────────────────────────────────────────────────────────────────────────
...
const getCallNotes = async (req, res, next) => {
    try {
        ...
        if (req.query.appointment_id) {
            conditions.push(`n.appointment_id = $${idx++}`);
            values.push(req.query.appointment_id);
        }
        if (req.query.room_id) {
            conditions.push(`n.room_id = $${idx++}`);
            values.push(req.query.room_id);
        }
        if (req.query.note_type) {
            conditions.push(`n.note_type = $${idx++}`);
            values.push(req.query.note_type);
        }
        const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
        const [rows, countRes] = await Promise.all([
            (0, database_1.query)(`SELECT n.*,
                p.first_name || ' ' || p.last_name AS patient_name,
                d.first_name || ' ' || d.last_name AS doctor_name
         FROM call_ai_notes n
         LEFT JOIN patients p ON p.id = n.patient_id
         ...
         LIMIT $${idx} OFFSET $${idx + 1}`, [...values, limit, offset]),
            (0, database_1.query)(`SELECT COUNT(*) FROM call_ai_notes n ${where}`, values),
        ]);
        res.json({
            success: true,
            data: rows.rows,
...
exports.getCallNotes = getCallNotes;
//# sourceMappingURL=call-notes.controller.js.map