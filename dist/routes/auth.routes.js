...
router.post('/register-lab-staff', auth_middleware_1.authenticate, (0, auth_middleware_1.authorize)('admin'), (0, express_validator_1.body)('name').notEmpty().trim().withMessage('name is required'), (0, express_validator_1.body)('branch_id').isInt().withMessage('branch_id is required'), (0, express_validator_1.body)('email').isEmail().withMessage('A valid email is required'), (0, express_validator_1.body)('password').isLength({ min: 8 }).withMessage('Password must be at least 8 characters'), validate_middleware_1.validate, authCtrl.registerLabStaff);
